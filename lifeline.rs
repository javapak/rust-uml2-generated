...
lazy_static! {
    static ref LIFELINE_REGISTRY: Mutex<RefCell<HashMap<String, Lifeline>>> = 
...
pub struct Lifeline {
    ...
    pub decomposed_as: Option<String>,
    pub covered_by: Vec<String>,
}

#[wasm_bindgen]
impl Lifeline {
    /// Creates a new Lifeline and returns its ID
    #[wasm_bindgen]
    pub fn create(interaction: String) -> String {
    ...
    /// Updates a Lifeline instance
    /// Takes a JavaScript object and updates the registry
    #[wasm_bindgen]
    pub fn update(value: JsValue) -> Result<(), JsValue> {
        let instance: Lifeline = serde_wasm_bindgen::from_value(value)
    ...
    /// Checks if a Lifeline exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all Lifeline instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<Lifeline> = LIFELINE_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of Lifeline instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        LIFELINE_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all Lifeline instances
    #[wasm_bindgen]
    pub fn clear_all() {
        LIFELINE_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds Lifeline instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: Lifeline = serde_wasm_bindgen::from_value(instance_js)
    ...
    pub fn set_interaction(instance_id: String, ref_id: String) -> Result<(), JsValue> {
        ...
        if let Ok(target_js) = Interaction::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Interaction>(target_js) {
                if !target.lifeline.contains(&instance_id) {
                    target.lifeline.push(instance_id.clone());
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
    ...
    /// Sets the selector reference
    #[wasm_bindgen]
    pub fn set_selector(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Lifeline = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        instance.selector = ref_id;
    ...
    /// Sets the decomposed_as reference
    #[wasm_bindgen]
    pub fn set_decomposed_as(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Lifeline = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        instance.decomposed_as = ref_id;
    ...
    /// Adds a InteractionFragment to covered_by
    #[wasm_bindgen]
    pub fn add_covered_by(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Lifeline = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if instance.covered_by.contains(&ref_id) {
            return Ok(false);
        }

        instance.covered_by.push(ref_id.clone());
        ...
        if let Ok(target_js) = InteractionFragment::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<InteractionFragment>(target_js) {
                if !target.covered.contains(&instance_id) {
                    target.covered.push(instance_id);
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
    ...
    /// Removes a InteractionFragment from covered_by
    #[wasm_bindgen]
    pub fn remove_covered_by(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Lifeline = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if let Some(pos) = instance.covered_by.iter().position(|x| x == &ref_id) {
            instance.covered_by.remove(pos);

        if let Ok(target_js) = InteractionFragment::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<InteractionFragment>(target_js) {
                target.covered.retain(|x| x != &instance_id);
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = InteractionFragment::update(target_js);
                }
            }
    ...
    /// Clears all InteractionFragment from covered_by
    #[wasm_bindgen]
    pub fn clear_covered_by(instance_id: String) -> Result<usize, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Lifeline = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        let count = instance.covered_by.len();

        // Update all opposite references
        for ref_id in &instance.covered_by {
            if let Ok(target_js) = InteractionFragment::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<InteractionFragment>(target_js) {
                target.covered.retain(|x| x != &instance_id);
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
...
impl Lifeline {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate selector reference exists
        ...
        // Validate decomposed_as reference exists
        if let Some(id) = &self.decomposed_as {
            if !PartDecomposition::exists(id.clone()) {
                errors.push(format!("PartDecomposition {} not found", id));
            }
        }

        // Validate all covered_by references exist
        for id in &self.covered_by {
