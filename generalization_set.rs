...
pub struct GeneralizationSet {
    ...
    is_covering: bool,
    is_disjoint: bool,
    powertype: Option<Rc<RefCell<Classifier>>>,
    generalization: Vec<Rc<RefCell<Generalization>>>,
}

impl GeneralizationSet {
    pub fn new(is_covering: bool, is_disjoint: bool) -> Self {
        Self {
    ...
    /// Sets is_covering
    pub fn set_is_covering(&mut self, value: bool) {
        self.is_covering = value;
    }
    ...
    /// Sets is_disjoint
    pub fn set_is_disjoint(&mut self, value: bool) {
        self.is_disjoint = value;
    }

    /// Returns a reference to powertype if present
    pub fn powertype(&self) -> Option<&Rc<RefCell<Classifier>>> {
        self.powertype.as_ref()
    }

    /// Returns a mutable reference to powertype if present
    pub fn powertype_mut(&mut self) -> Option<&mut Rc<RefCell<Classifier>>> {
        self.powertype.as_mut()
    }

    /// Sets powertype
    pub fn set_powertype(&mut self, value: Rc<RefCell<Classifier>>) {
        self.powertype = Some(value);
    }

    /// Takes powertype, leaving None in its place
    pub fn take_powertype(&mut self) -> Option<Rc<RefCell<Classifier>>> {
        self.powertype.take()
    }

    /// Returns a reference to generalization
    pub fn generalization(&self) -> &Vec<Rc<RefCell<Generalization>>> {
        &self.generalization
    }

    /// Returns a mutable reference to generalization
    pub fn generalization_mut(&mut self) -> &mut Vec<Rc<RefCell<Generalization>>> {
        &mut self.generalization
    }

    /// Adds an item to generalization
    pub fn add_generalization(&mut self, item: Rc<RefCell<Generalization>>) {
...
impl std::fmt::Display for GeneralizationSet {
