...
lazy_static! {
    static ref GENERALIZATION_SET_REGISTRY: Mutex<RefCell<HashMap<String, GeneralizationSet>>> = 
...
impl GeneralizationSet {
    /// Creates a new GeneralizationSet and returns its ID
    #[wasm_bindgen]
    pub fn create(is_covering: String, is_disjoint: String) -> String {
        ...
        GENERALIZATION_SET_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Gets a GeneralizationSet by ID
    ...
    /// Updates a GeneralizationSet instance
    ...
    /// Deletes a GeneralizationSet by ID
    ...
    /// Checks if a GeneralizationSet exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all GeneralizationSet instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<GeneralizationSet> = GENERALIZATION_SET_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of GeneralizationSet instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        GENERALIZATION_SET_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all GeneralizationSet instances
    #[wasm_bindgen]
    pub fn clear_all() {
        GENERALIZATION_SET_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds GeneralizationSet instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: GeneralizationSet = serde_wasm_bindgen::from_value(instance_js)
    ...
    pub fn set_powertype(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        ...
        if let Some(old_ref_id) = &instance.powertype {
            if let Ok(target_js) = Classifier::get(old_ref_id.clone()) {
                if let Ok(mut target) = serde_wasm_bindgen::from_value::<Classifier>(target_js) {
                    target.powertype_extent.retain(|x| x != &instance_id);
                    if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
        ...
        // Add to new opposite
        if let Some(new_ref_id) = &ref_id {
        if let Ok(target_js) = Classifier::get(new_ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Classifier>(target_js) {
                if !target.powertype_extent.contains(&instance_id) {
                    target.powertype_extent.push(instance_id.clone());
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = Classifier::update(target_js);
                }
        ...
        if let Ok(target_js) = Generalization::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Generalization>(target_js) {
                if !target.generalization_set.contains(&instance_id) {
                    target.generalization_set.push(instance_id);
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
    ...
    pub fn remove_generalization(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Ok(target_js) = Generalization::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Generalization>(target_js) {
                target.generalization_set.retain(|x| x != &instance_id);
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = Generalization::update(target_js);
                }
            }
...
impl GeneralizationSet {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate powertype reference exists
        if let Some(id) = &self.powertype {
