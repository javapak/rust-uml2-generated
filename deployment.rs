...
#[wasm_bindgen]
impl Deployment {
    pub fn new(supplier: Vec<String>, client: Vec<String>, location: String) -> Self {
        Self {
    ...
    /// Returns a clone of deployed_artifact
    pub fn deployed_artifact(&self) -> Vec<String> {
        self.deployed_artifact.clone()
    }

    /// Adds an existing DeployedArtifact to deployed_artifact by ID
    pub fn add_deployed_artifact_by_id(&mut self, id: String) {
        self.deployed_artifact.push(id);
    }
    ...
    /// Takes ownership of location, replacing it with an empty string
