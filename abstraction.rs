// Abstraction - Generated UML Class
...
const TYPE_NAME: &str = "Abstraction";
...
pub fn clear_visibility(instance_id: String) -> Result<(), JsValue> {
    ...
    #[wasm_bindgen]
    pub fn clear_mapping(instance_id: String) -> Result<bool, JsValue> {
        let mut instance: Self = registry::get(&instance_id)
            .ok_or_else(|| JsValue::from_str("Instance not found"))?;
        
        if instance.mapping.is_none() {
            return Ok(false);
        }
        
        instance.mapping = None;
