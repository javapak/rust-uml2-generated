...
pub struct EnumerationLiteral {
    ...
    classifier: Vec<Rc<RefCell<Classifier>>>,
    slot: Vec<Slot>,
    specification: Option<ValueSpecification>,
    enumeration: Option<Weak<RefCell<Enumeration>>>,
}
...
impl EnumerationLiteral {
    ...
    /// Returns a reference to classifier
    pub fn classifier(&self) -> &Vec<Rc<RefCell<Classifier>>> {
        &self.classifier
    }

    /// Returns a mutable reference to classifier
    pub fn classifier_mut(&mut self) -> &mut Vec<Rc<RefCell<Classifier>>> {
        &mut self.classifier
    }

    /// Adds an item to classifier
    pub fn add_classifier(&mut self, item: Rc<RefCell<Classifier>>) {
        self.classifier.push(item);
    }

    /// Clears all items from classifier
    pub fn clear_classifier(&mut self) {
        self.classifier.clear();
    }

    /// Returns a slice of slot
    pub fn slot(&self) -> &[Slot] {
        &self.slot
    }

    /// Returns a mutable reference to slot
    pub fn slot_mut(&mut self) -> &mut Vec<Slot> {
        &mut self.slot
    }

    /// Adds an item to slot
    pub fn add_slot(&mut self, item: Slot) {
        self.slot.push(item);
    }

    /// Clears all items from slot
    pub fn clear_slot(&mut self) {
        self.slot.clear();
    }

    /// Returns a reference to specification if present
    pub fn specification(&self) -> Option<&ValueSpecification> {
        self.specification.as_ref()
    }

    /// Returns a mutable reference to specification if present
    pub fn specification_mut(&mut self) -> Option<&mut ValueSpecification> {
    ...
    pub fn take_specification(&mut self) -> Option<ValueSpecification> {
        self.specification.take()
    }

    /// Returns a reference to enumeration if present
    pub fn enumeration(&self) -> Option<&Weak<RefCell<Enumeration>>> {
        self.enumeration.as_ref()
    }

    /// Returns a mutable reference to enumeration if present
    pub fn enumeration_mut(&mut self) -> Option<&mut Weak<RefCell<Enumeration>>> {
        self.enumeration.as_mut()
    }

    /// Sets enumeration
    pub fn set_enumeration(&mut self, value: Weak<RefCell<Enumeration>>) {
        self.enumeration = Some(value);
    }

    /// Takes enumeration, leaving None in its place
    pub fn take_enumeration(&mut self) -> Option<Weak<RefCell<Enumeration>>> {
        self.enumeration.take()
    }

}

impl std::fmt::Display for EnumerationLiteral {
