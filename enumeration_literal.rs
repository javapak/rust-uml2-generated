...
impl EnumerationLiteral {
    ...
    /// Adds an existing Classifier to classifier by ID
    pub fn add_classifier_by_id(&mut self, id: String) {
        self.classifier.push(id);
    }
    ...
    /// Returns a clone of enumeration if present
    pub fn enumeration(&self) -> Option<String> {
        self.enumeration.clone()
    }

    /// Sets enumeration
    pub fn set_enumeration(&mut self, value: String) {
        self.enumeration = Some(value);
    }

    /// Takes enumeration, leaving None in its place
    pub fn take_enumeration(&mut self) -> Option<String> {
