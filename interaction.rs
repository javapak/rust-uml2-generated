...
lazy_static! {
    static ref INTERACTION_REGISTRY: Mutex<RefCell<HashMap<String, Interaction>>> = 
...
impl Interaction {
    ...
    pub fn create(is_leaf: String, is_abstract: String, is_active: String, is_reentrant: String) -> String {
        ...
        INTERACTION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Updates a Interaction instance
    ...
    /// Checks if a Interaction exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all Interaction instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<Interaction> = INTERACTION_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of Interaction instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        INTERACTION_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all Interaction instances
    #[wasm_bindgen]
    pub fn clear_all() {
        INTERACTION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds Interaction instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: Interaction = serde_wasm_bindgen::from_value(instance_js)
    ...
    pub fn add_lifeline(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Ok(target_js) = Lifeline::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Lifeline>(target_js) {
                target.interaction = instance_id;
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = Lifeline::update(target_js);
                }
            }
    ...
    /// Removes a Lifeline from lifeline
    #[wasm_bindgen]
    pub fn remove_lifeline(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(pos) = instance.lifeline.iter().position(|x| x == &ref_id) {
            instance.lifeline.remove(pos);
    ...
    /// Clears all Lifeline from lifeline
    #[wasm_bindgen]
    ...
    pub fn add_fragment(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Ok(target_js) = InteractionFragment::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<InteractionFragment>(target_js) {
                target.enclosing_interaction = Some(instance_id);
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
    ...
    pub fn remove_action(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(pos) = instance.action.iter().position(|x| x == &ref_id) {
            instance.action.remove(pos);
    ...
    /// Removes a Gate from formal_gate
    #[wasm_bindgen]
    pub fn remove_formal_gate(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(pos) = instance.formal_gate.iter().position(|x| x == &ref_id) {
            instance.formal_gate.remove(pos);
    ...
    /// Clears all Gate from formal_gate
    #[wasm_bindgen]
    ...
    pub fn add_message(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Ok(target_js) = Message::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Message>(target_js) {
                target.interaction = instance_id;
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = Message::update(target_js);
                }
            }
        }
    ...
    /// Removes a Message from message
    #[wasm_bindgen]
    pub fn remove_message(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(pos) = instance.message.iter().position(|x| x == &ref_id) {
            instance.message.remove(pos);
    ...
    /// Clears all Message from message
    #[wasm_bindgen]
...
impl Interaction {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate all lifeline references exist
        for id in &self.lifeline {
            if !Lifeline::exists(id.clone()) {
                errors.push(format!("Lifeline {} not found", id));
            }
        }
        ...
        // Validate all action references exist
        for id in &self.action {
            if !Action::exists(id.clone()) {
                errors.push(format!("Action {} not found", id));
            }
        }

        // Validate all formal_gate references exist
        for id in &self.formal_gate {
            if !Gate::exists(id.clone()) {
                errors.push(format!("Gate {} not found", id));
            }
        }

        // Validate all message references exist
        for id in &self.message {
