...
// ============================================================================
//
// Type:           InterfaceRealization (struct)
// Source Package: uml
...
use crate::behaviored_classifier::BehavioredClassifier;

lazy_static! {
    static ref INTERFACE_REALIZATION_REGISTRY: Mutex<RefCell<HashMap<String, InterfaceRealization>>> = 
        Mutex::new(RefCell::new(HashMap::new()));
}

#[derive(Debug, Clone, Serialize, Deserialize, Tsify)]
pub struct InterfaceRealization {
    ...
    pub contract: String,
    pub implementing_classifier: String,
}

#[wasm_bindgen]
impl InterfaceRealization {
    /// Creates a new InterfaceRealization and returns its ID
    #[wasm_bindgen]
    pub fn create(contract: String, implementing_classifier: String) -> String {
        ...
        let instance = Self {
            ...
            mapping: None,
            contract: contract,
            implementing_classifier: implementing_classifier,
        };

        INTERFACE_REALIZATION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Gets a InterfaceRealization by ID
    ...
    /// Updates a InterfaceRealization instance
    /// Takes a JavaScript object and updates the registry
    #[wasm_bindgen]
    pub fn update(value: JsValue) -> Result<(), JsValue> {
        let instance: InterfaceRealization = serde_wasm_bindgen::from_value(value)
    ...
    /// Deletes a InterfaceRealization by ID
    ...
    /// Checks if a InterfaceRealization exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all InterfaceRealization instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<InterfaceRealization> = INTERFACE_REALIZATION_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of InterfaceRealization instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        INTERFACE_REALIZATION_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all InterfaceRealization instances
    #[wasm_bindgen]
    pub fn clear_all() {
        INTERFACE_REALIZATION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds InterfaceRealization instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: InterfaceRealization = serde_wasm_bindgen::from_value(instance_js)
    ...
    /// Adds a NamedElement to supplier
    #[wasm_bindgen]
    pub fn add_supplier(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: InterfaceRealization = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if instance.supplier.contains(&ref_id) {
            return Ok(false);
        }

        instance.supplier.push(ref_id.clone());
    ...
    /// Removes a NamedElement from supplier
    #[wasm_bindgen]
    pub fn remove_supplier(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: InterfaceRealization = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if let Some(pos) = instance.supplier.iter().position(|x| x == &ref_id) {
            instance.supplier.remove(pos);
    ...
    /// Clears all NamedElement from supplier
    #[wasm_bindgen]
    pub fn clear_supplier(instance_id: String) -> Result<usize, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: InterfaceRealization = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        let count = instance.supplier.len();
    ...
    /// Adds a NamedElement to client
    #[wasm_bindgen]
    pub fn add_client(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: InterfaceRealization = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if instance.client.contains(&ref_id) {
            return Ok(false);
        }

        instance.client.push(ref_id.clone());

        if let Ok(target_js) = NamedElement::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<NamedElement>(target_js) {
                if !target.client_dependency.contains(&instance_id) {
                    target.client_dependency.push(instance_id);
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = NamedElement::update(target_js);
                }
            }
        }
    ...
    /// Removes a NamedElement from client
    #[wasm_bindgen]
    pub fn remove_client(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: InterfaceRealization = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if let Some(pos) = instance.client.iter().position(|x| x == &ref_id) {
            instance.client.remove(pos);

        if let Ok(target_js) = NamedElement::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<NamedElement>(target_js) {
                target.client_dependency.retain(|x| x != &instance_id);
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = NamedElement::update(target_js);
                }
            }
    ...
    /// Clears all NamedElement from client
    #[wasm_bindgen]
    pub fn clear_client(instance_id: String) -> Result<usize, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: InterfaceRealization = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        let count = instance.client.len();

        // Update all opposite references
        for ref_id in &instance.client {
            if let Ok(target_js) = NamedElement::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<NamedElement>(target_js) {
                target.client_dependency.retain(|x| x != &instance_id);
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
    ...
    /// Sets the mapping reference
    #[wasm_bindgen]
    pub fn set_mapping(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: InterfaceRealization = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        instance.mapping = ref_id;
    ...
    /// Sets the implementing_classifier reference
    #[wasm_bindgen]
    pub fn set_implementing_classifier(instance_id: String, ref_id: String) -> Result<(), JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: InterfaceRealization = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if let Ok(target_js) = BehavioredClassifier::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<BehavioredClassifier>(target_js) {
                if !target.interface_realization.contains(&instance_id) {
                    target.interface_realization.push(instance_id.clone());
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = BehavioredClassifier::update(target_js);
                }
            }
        }

        instance.implementing_classifier = ref_id;
...
impl InterfaceRealization {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate all supplier references exist
        for id in &self.supplier {
            if !NamedElement::exists(id.clone()) {
                errors.push(format!("NamedElement {} not found", id));
            }
        }

        // Validate all client references exist
        for id in &self.client {
            if !NamedElement::exists(id.clone()) {
                errors.push(format!("NamedElement {} not found", id));
            }
        }

        // Validate mapping reference exists
        if let Some(id) = &self.mapping {
            if !OpaqueExpression::exists(id.clone()) {
                errors.push(format!("OpaqueExpression {} not found", id));
            }
        }

        // Validate contract reference exists
        if !Interface::exists(self.contract.clone()) {
            errors.push(format!("Interface {} not found", self.contract));
        }

        // Validate implementing_classifier reference exists
        if !BehavioredClassifier::exists(self.implementing_classifier.clone()) {
            errors.push(format!("BehavioredClassifier {} not found", self.implementing_classifier));
        }
