...
pub struct InterfaceRealization {
    ...
    contract: Rc<RefCell<Interface>>,
    implementing_classifier: Weak<RefCell<BehavioredClassifier>>,
}

impl InterfaceRealization {
    pub fn new(supplier: Vec<Rc<RefCell<NamedElement>>>, client: Vec<Rc<RefCell<NamedElement>>>, contract: Rc<RefCell<Interface>>, implementing_classifier: Weak<RefCell<BehavioredClassifier>>) -> Self {
        Self {
    ...
    pub fn contract(&self) -> &Rc<RefCell<Interface>> {
        &self.contract
    }

    /// Returns a mutable reference to contract
    pub fn contract_mut(&mut self) -> &mut Rc<RefCell<Interface>> {
        &mut self.contract
    }

    /// Sets contract
    pub fn set_contract(&mut self, value: Rc<RefCell<Interface>>) {
        self.contract = value;
    }

    /// Returns a reference to implementing_classifier
    pub fn implementing_classifier(&self) -> &Weak<RefCell<BehavioredClassifier>> {
        &self.implementing_classifier
    }

    /// Returns a mutable reference to implementing_classifier
    pub fn implementing_classifier_mut(&mut self) -> &mut Weak<RefCell<BehavioredClassifier>> {
        &mut self.implementing_classifier
    }

    /// Sets implementing_classifier
    pub fn set_implementing_classifier(&mut self, value: Weak<RefCell<BehavioredClassifier>>) {
        self.implementing_classifier = value;
    }

}

impl Default for InterfaceRealization {
    fn default() -> Self {
        Self {
            e_annotations: Vec::new(),
            owned_comment: Vec::new(),
            ...
            implementing_classifier: Default::default(),
        }
    }
}

impl std::fmt::Display for InterfaceRealization {
