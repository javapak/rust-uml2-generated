...
impl SendSignalAction {
    pub fn new(is_leaf: bool, target: InputPin, signal: Rc<RefCell<Signal>>) -> Self {
        Self {
    ...
    /// Returns a reference to signal
    pub fn signal(&self) -> &Rc<RefCell<Signal>> {
        &self.signal
    }

    /// Returns a mutable reference to signal
    pub fn signal_mut(&mut self) -> &mut Rc<RefCell<Signal>> {
        &mut self.signal
    }

    /// Sets signal
    pub fn set_signal(&mut self, value: Rc<RefCell<Signal>>) {
        self.signal = value;
    }

}

impl Default for SendSignalAction {
    fn default() -> Self {
        Self {
            e_annotations: Vec::new(),
            owned_comment: Vec::new(),
...
impl std::fmt::Display for SendSignalAction {
