...
lazy_static! {
    static ref VARIABLE_REGISTRY: Mutex<RefCell<HashMap<String, Variable>>> = 
...
impl Variable {
    ...
    #[wasm_bindgen]
    pub fn create(is_ordered: String, is_unique: String) -> String {
    ...
    /// Updates a Variable instance
    /// Takes a JavaScript object and updates the registry
    #[wasm_bindgen]
    pub fn update(value: JsValue) -> Result<(), JsValue> {
        let instance: Variable = serde_wasm_bindgen::from_value(value)
    ...
    /// Checks if a Variable exists by ID
    #[wasm_bindgen]
    ...
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<Variable> = VARIABLE_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of Variable instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        VARIABLE_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all Variable instances
    #[wasm_bindgen]
    pub fn clear_all() {
        VARIABLE_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds Variable instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: Variable = serde_wasm_bindgen::from_value(instance_js)
    ...
    /// Sets the scope reference
    #[wasm_bindgen]
    pub fn set_scope(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Variable = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        // Remove from old opposite
        if let Some(old_ref_id) = &instance.scope {
            if let Ok(target_js) = StructuredActivityNode::get(old_ref_id.clone()) {
                if let Ok(mut target) = serde_wasm_bindgen::from_value::<StructuredActivityNode>(target_js) {
                    target.variable.retain(|x| x != &instance_id);
                    if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
        ...
        if let Ok(target_js) = StructuredActivityNode::get(new_ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<StructuredActivityNode>(target_js) {
                if !target.variable.contains(&instance_id) {
                    target.variable.push(instance_id.clone());
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = StructuredActivityNode::update(target_js);
                }
    ...
    /// Sets the activity_scope reference
    #[wasm_bindgen]
    pub fn set_activity_scope(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Variable = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        // Remove from old opposite
        if let Some(old_ref_id) = &instance.activity_scope {
        ...
        instance.activity_scope = ref_id;
...
impl Variable {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate scope reference exists
        ...
        // Validate activity_scope reference exists
        if let Some(id) = &self.activity_scope {
