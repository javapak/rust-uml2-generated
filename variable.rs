...
pub struct Variable {
    ...
    scope: Option<Weak<RefCell<StructuredActivityNode>>>,
    activity_scope: Option<Weak<RefCell<Activity>>>,
}
...
impl Variable {
    ...
    /// Returns a reference to scope if present
    pub fn scope(&self) -> Option<&Weak<RefCell<StructuredActivityNode>>> {
        self.scope.as_ref()
    }

    /// Returns a mutable reference to scope if present
    pub fn scope_mut(&mut self) -> Option<&mut Weak<RefCell<StructuredActivityNode>>> {
        self.scope.as_mut()
    }

    /// Sets scope
    pub fn set_scope(&mut self, value: Weak<RefCell<StructuredActivityNode>>) {
        self.scope = Some(value);
    }

    /// Takes scope, leaving None in its place
    pub fn take_scope(&mut self) -> Option<Weak<RefCell<StructuredActivityNode>>> {
        self.scope.take()
    }

    /// Returns a reference to activity_scope if present
    pub fn activity_scope(&self) -> Option<&Weak<RefCell<Activity>>> {
        self.activity_scope.as_ref()
    }

    /// Returns a mutable reference to activity_scope if present
    pub fn activity_scope_mut(&mut self) -> Option<&mut Weak<RefCell<Activity>>> {
        self.activity_scope.as_mut()
    }

    /// Sets activity_scope
    pub fn set_activity_scope(&mut self, value: Weak<RefCell<Activity>>) {
        self.activity_scope = Some(value);
    }

    /// Takes activity_scope, leaving None in its place
    pub fn take_activity_scope(&mut self) -> Option<Weak<RefCell<Activity>>> {
        self.activity_scope.take()
    }
