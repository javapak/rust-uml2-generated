...
pub struct ProtocolStateMachine {
    ...
    region: Vec<Region>,
    submachine_state: Vec<Rc<RefCell<State>>>,
    connection_point: Vec<Pseudostate>,
    extended_state_machine: Vec<Rc<RefCell<StateMachine>>>,
    conformance: Vec<ProtocolConformance>,
}

impl ProtocolStateMachine {
    pub fn new(is_leaf: bool, is_abstract: bool, is_active: bool, is_reentrant: bool, region: Vec<Region>) -> Self {
        Self {
    ...
    /// Returns a reference to submachine_state
    pub fn submachine_state(&self) -> &Vec<Rc<RefCell<State>>> {
        &self.submachine_state
    }

    /// Returns a mutable reference to submachine_state
    pub fn submachine_state_mut(&mut self) -> &mut Vec<Rc<RefCell<State>>> {
        &mut self.submachine_state
    }

    /// Adds an item to submachine_state
    pub fn add_submachine_state(&mut self, item: Rc<RefCell<State>>) {
        self.submachine_state.push(item);
    }

    /// Clears all items from submachine_state
    pub fn clear_submachine_state(&mut self) {
        self.submachine_state.clear();
    }
    ...
    /// Returns a reference to extended_state_machine
    pub fn extended_state_machine(&self) -> &Vec<Rc<RefCell<StateMachine>>> {
        &self.extended_state_machine
    }

    /// Returns a mutable reference to extended_state_machine
    pub fn extended_state_machine_mut(&mut self) -> &mut Vec<Rc<RefCell<StateMachine>>> {
        &mut self.extended_state_machine
    }

    /// Adds an item to extended_state_machine
    pub fn add_extended_state_machine(&mut self, item: Rc<RefCell<StateMachine>>) {
        self.extended_state_machine.push(item);
    }

    /// Clears all items from extended_state_machine
    pub fn clear_extended_state_machine(&mut self) {
        self.extended_state_machine.clear();
    }

    /// Returns a slice of conformance
    pub fn conformance(&self) -> &[ProtocolConformance] {
        &self.conformance
    }

    /// Returns a mutable reference to conformance
    pub fn conformance_mut(&mut self) -> &mut Vec<ProtocolConformance> {
        &mut self.conformance
    }

    /// Adds an item to conformance
    pub fn add_conformance(&mut self, item: ProtocolConformance) {
        self.conformance.push(item);
    }

    /// Clears all items from conformance
    pub fn clear_conformance(&mut self) {
        self.conformance.clear();
    }

}

impl std::fmt::Display for ProtocolStateMachine {
