...
lazy_static! {
    static ref GENERAL_ORDERING_REGISTRY: Mutex<RefCell<HashMap<String, GeneralOrdering>>> = 
...
impl GeneralOrdering {
    /// Creates a new GeneralOrdering and returns its ID
    #[wasm_bindgen]
    pub fn create(before: String, after: String) -> String {
        ...
        GENERAL_ORDERING_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Updates a GeneralOrdering instance
    ...
    /// Deletes a GeneralOrdering by ID
    ...
    /// Checks if a GeneralOrdering exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all GeneralOrdering instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<GeneralOrdering> = GENERAL_ORDERING_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of GeneralOrdering instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        GENERAL_ORDERING_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all GeneralOrdering instances
    #[wasm_bindgen]
    pub fn clear_all() {
        GENERAL_ORDERING_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds GeneralOrdering instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: GeneralOrdering = serde_wasm_bindgen::from_value(instance_js)
    ...
    pub fn set_before(instance_id: String, ref_id: String) -> Result<(), JsValue> {
        ...
        if let Ok(target_js) = OccurrenceSpecification::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<OccurrenceSpecification>(target_js) {
                if !target.to_after.contains(&instance_id) {
                    target.to_after.push(instance_id.clone());
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = OccurrenceSpecification::update(target_js);
                }
            }
        }

        instance.before = ref_id;
    ...
    pub fn set_after(instance_id: String, ref_id: String) -> Result<(), JsValue> {
        ...
        if let Ok(target_js) = OccurrenceSpecification::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<OccurrenceSpecification>(target_js) {
                if !target.to_before.contains(&instance_id) {
                    target.to_before.push(instance_id.clone());
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = OccurrenceSpecification::update(target_js);
                }
...
impl GeneralOrdering {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate before reference exists
        if !OccurrenceSpecification::exists(self.before.clone()) {
            errors.push(format!("OccurrenceSpecification {} not found", self.before));
        }

        // Validate after reference exists
        if !OccurrenceSpecification::exists(self.after.clone()) {
            errors.push(format!("OccurrenceSpecification {} not found", self.after));
        }
