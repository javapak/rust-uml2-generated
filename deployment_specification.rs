...
lazy_static! {
    static ref DEPLOYMENT_SPECIFICATION_REGISTRY: Mutex<RefCell<HashMap<String, DeploymentSpecification>>> = 
...
impl DeploymentSpecification {
    /// Creates a new DeploymentSpecification and returns its ID
    #[wasm_bindgen]
    pub fn create(is_leaf: String, is_abstract: String) -> String {
        ...
        DEPLOYMENT_SPECIFICATION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Gets a DeploymentSpecification by ID
    ...
    /// Updates a DeploymentSpecification instance
    ...
    /// Deletes a DeploymentSpecification by ID
    ...
    /// Checks if a DeploymentSpecification exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all DeploymentSpecification instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<DeploymentSpecification> = DEPLOYMENT_SPECIFICATION_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of DeploymentSpecification instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        DEPLOYMENT_SPECIFICATION_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all DeploymentSpecification instances
    #[wasm_bindgen]
    pub fn clear_all() {
        DEPLOYMENT_SPECIFICATION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds DeploymentSpecification instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: DeploymentSpecification = serde_wasm_bindgen::from_value(instance_js)
    ...
    pub fn set_deployment(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        ...
        if let Some(old_ref_id) = &instance.deployment {
            if let Ok(target_js) = Deployment::get(old_ref_id.clone()) {
                if let Ok(mut target) = serde_wasm_bindgen::from_value::<Deployment>(target_js) {
                    target.configuration.retain(|x| x != &instance_id);
                    if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
        ...
        // Add to new opposite
        if let Some(new_ref_id) = &ref_id {
        if let Ok(target_js) = Deployment::get(new_ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Deployment>(target_js) {
                if !target.configuration.contains(&instance_id) {
                    target.configuration.push(instance_id.clone());
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = Deployment::update(target_js);
                }
    ...
    /// Sets the deployment_location field
    #[wasm_bindgen]
    pub fn set_deployment_location(instance_id: String, value: Option<String>) -> Result<(), JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: DeploymentSpecification = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        instance.deployment_location = value;
    ...
    /// Sets the execution_location field
    #[wasm_bindgen]
    pub fn set_execution_location(instance_id: String, value: Option<String>) -> Result<(), JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: DeploymentSpecification = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        instance.execution_location = value;
...
impl DeploymentSpecification {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate deployment reference exists
        if let Some(id) = &self.deployment {
