...
pub struct MessageOccurrenceSpecification {
    ...
    message: Option<Rc<RefCell<Message>>>,
}
...
impl MessageOccurrenceSpecification {
    ...
    /// Returns a reference to message if present
    pub fn message(&self) -> Option<&Rc<RefCell<Message>>> {
        self.message.as_ref()
    }

    /// Returns a mutable reference to message if present
    pub fn message_mut(&mut self) -> Option<&mut Rc<RefCell<Message>>> {
        self.message.as_mut()
    }

    /// Sets message
    pub fn set_message(&mut self, value: Rc<RefCell<Message>>) {
        self.message = Some(value);
    }

    /// Takes message, leaving None in its place
    pub fn take_message(&mut self) -> Option<Rc<RefCell<Message>>> {
        self.message.take()
    }
