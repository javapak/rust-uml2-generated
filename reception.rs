...
#[wasm_bindgen]
impl Reception {
    /// Creates a new Reception instance
    #[wasm_bindgen]
...
pub fn set_concurrency(instance_id: String, value: CallConcurrencyKind) -> Result<(), JsValue> {
    ...
    #[wasm_bindgen]
    pub fn clear_signal(instance_id: String) -> Result<bool, JsValue> {
        let mut instance: Self = registry::get(&instance_id)
            .ok_or_else(|| JsValue::from_str("Instance not found"))?;
        
        if instance.signal.is_none() {
            return Ok(false);
        }
        
        instance.signal = None;
