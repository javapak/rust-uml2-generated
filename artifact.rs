...
pub struct Artifact {
    ...
    nested_artifact: Vec<Box<Artifact>>,
...
impl Artifact {
    ...
    /// Returns a reference to nested_artifact
    pub fn nested_artifact(&self) -> &Vec<Box<Artifact>> {
        &self.nested_artifact
    }

    /// Returns a mutable reference to nested_artifact
    pub fn nested_artifact_mut(&mut self) -> &mut Vec<Box<Artifact>> {
        &mut self.nested_artifact
    }

    /// Adds an item to nested_artifact
    pub fn add_nested_artifact(&mut self, item: Artifact) {
        self.nested_artifact.push(Box::new(item));
    }
