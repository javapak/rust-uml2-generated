// DeploymentTarget - Generated UML Class
...
const TYPE_NAME: &str = "DeploymentTarget";
...
impl DeploymentTarget {
    /// Creates a new DeploymentTarget instance
    #[wasm_bindgen]
...
pub fn clear_visibility(instance_id: String) -> Result<(), JsValue> {
    ...
    pub fn add_deployment(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(mut target) = registry::get::<Deployment>(&ref_id) {
            target.location = instance_id.clone();
            let _ = registry::insert(ref_id.clone(), "Deployment", &target);
        }
    ...
    pub fn remove_deployment(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let initial_len = instance.deployment.len();
        instance.deployment.retain(|id| id != &ref_id);
        let removed = instance.deployment.len() < initial_len;
        ...
        // Update opposite reference
        if let Some(target) = registry::get::<Deployment>(&ref_id) {
            let _ = registry::insert(ref_id.clone(), "Deployment", &target);
        }
