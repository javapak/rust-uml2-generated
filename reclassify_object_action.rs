...
pub struct ReclassifyObjectAction {
    ...
    is_replace_all: bool,
    old_classifier: Vec<Rc<RefCell<Classifier>>>,
    new_classifier: Vec<Rc<RefCell<Classifier>>>,
    object: InputPin,
}

impl ReclassifyObjectAction {
    pub fn new(is_leaf: bool, is_replace_all: bool, object: InputPin) -> Self {
        Self {
    ...
    /// Returns a reference to old_classifier
    pub fn old_classifier(&self) -> &Vec<Rc<RefCell<Classifier>>> {
        &self.old_classifier
    }

    /// Returns a mutable reference to old_classifier
    pub fn old_classifier_mut(&mut self) -> &mut Vec<Rc<RefCell<Classifier>>> {
        &mut self.old_classifier
    }

    /// Adds an item to old_classifier
    pub fn add_old_classifier(&mut self, item: Rc<RefCell<Classifier>>) {
        self.old_classifier.push(item);
    }

    /// Clears all items from old_classifier
    pub fn clear_old_classifier(&mut self) {
        self.old_classifier.clear();
    }

    /// Returns a reference to new_classifier
    pub fn new_classifier(&self) -> &Vec<Rc<RefCell<Classifier>>> {
        &self.new_classifier
    }

    /// Returns a mutable reference to new_classifier
    pub fn new_classifier_mut(&mut self) -> &mut Vec<Rc<RefCell<Classifier>>> {
        &mut self.new_classifier
    }

    /// Adds an item to new_classifier
    pub fn add_new_classifier(&mut self, item: Rc<RefCell<Classifier>>) {
        self.new_classifier.push(item);
    }

    /// Clears all items from new_classifier
    pub fn clear_new_classifier(&mut self) {
        self.new_classifier.clear();
    }
...
impl Default for ReclassifyObjectAction {
    fn default() -> Self {
        Self {
            e_annotations: Vec::new(),
            owned_comment: Vec::new(),
...
impl std::fmt::Display for ReclassifyObjectAction {
