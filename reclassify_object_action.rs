...
// ============================================================================
//
// Type:           ReclassifyObjectAction (struct)
// Source Package: uml
...
lazy_static! {
    static ref RECLASSIFY_OBJECT_ACTION_REGISTRY: Mutex<RefCell<HashMap<String, ReclassifyObjectAction>>> = 
        Mutex::new(RefCell::new(HashMap::new()));
}

#[derive(Debug, Clone, Serialize, Deserialize, Tsify)]
pub struct ReclassifyObjectAction {
    ...
    pub old_classifier: Vec<String>,
    pub new_classifier: Vec<String>,
    pub object: String,
}

#[wasm_bindgen]
impl ReclassifyObjectAction {
    /// Creates a new ReclassifyObjectAction and returns its ID
    #[wasm_bindgen]
    pub fn create(is_leaf: String, is_replace_all: String, object: String) -> String {
        ...
        let instance = Self {
            ...
            old_classifier: Vec::new(),
            new_classifier: Vec::new(),
            object: object,
        };

        RECLASSIFY_OBJECT_ACTION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Gets a ReclassifyObjectAction by ID
    ...
    /// Updates a ReclassifyObjectAction instance
    /// Takes a JavaScript object and updates the registry
    #[wasm_bindgen]
    pub fn update(value: JsValue) -> Result<(), JsValue> {
        let instance: ReclassifyObjectAction = serde_wasm_bindgen::from_value(value)
    ...
    /// Deletes a ReclassifyObjectAction by ID
    ...
    /// Checks if a ReclassifyObjectAction exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all ReclassifyObjectAction instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<ReclassifyObjectAction> = RECLASSIFY_OBJECT_ACTION_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of ReclassifyObjectAction instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        RECLASSIFY_OBJECT_ACTION_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all ReclassifyObjectAction instances
    #[wasm_bindgen]
    pub fn clear_all() {
        RECLASSIFY_OBJECT_ACTION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds ReclassifyObjectAction instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: ReclassifyObjectAction = serde_wasm_bindgen::from_value(instance_js)
    ...
    /// Adds a Classifier to old_classifier
    #[wasm_bindgen]
    pub fn add_old_classifier(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ReclassifyObjectAction = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if instance.old_classifier.contains(&ref_id) {
            return Ok(false);
        }

        instance.old_classifier.push(ref_id.clone());
    ...
    /// Removes a Classifier from old_classifier
    #[wasm_bindgen]
    pub fn remove_old_classifier(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ReclassifyObjectAction = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if let Some(pos) = instance.old_classifier.iter().position(|x| x == &ref_id) {
            instance.old_classifier.remove(pos);
    ...
    /// Clears all Classifier from old_classifier
    #[wasm_bindgen]
    pub fn clear_old_classifier(instance_id: String) -> Result<usize, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ReclassifyObjectAction = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        let count = instance.old_classifier.len();

        instance.old_classifier.clear();
    ...
    /// Adds a Classifier to new_classifier
    #[wasm_bindgen]
    pub fn add_new_classifier(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ReclassifyObjectAction = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if instance.new_classifier.contains(&ref_id) {
            return Ok(false);
        }

        instance.new_classifier.push(ref_id.clone());
    ...
    /// Removes a Classifier from new_classifier
    #[wasm_bindgen]
    pub fn remove_new_classifier(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ReclassifyObjectAction = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if let Some(pos) = instance.new_classifier.iter().position(|x| x == &ref_id) {
            instance.new_classifier.remove(pos);
    ...
    /// Clears all Classifier from new_classifier
    #[wasm_bindgen]
    pub fn clear_new_classifier(instance_id: String) -> Result<usize, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ReclassifyObjectAction = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        let count = instance.new_classifier.len();

        instance.new_classifier.clear();
...
impl ReclassifyObjectAction {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate all old_classifier references exist
        for id in &self.old_classifier {
            if !Classifier::exists(id.clone()) {
                errors.push(format!("Classifier {} not found", id));
            }
        }

        // Validate all new_classifier references exist
        for id in &self.new_classifier {
            if !Classifier::exists(id.clone()) {
                errors.push(format!("Classifier {} not found", id));
            }
        }
