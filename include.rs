...
lazy_static! {
    static ref INCLUDE_REGISTRY: Mutex<RefCell<HashMap<String, Include>>> = 
...
pub struct Include {
    ...
    pub addition: String,
    pub including_case: String,
}

#[wasm_bindgen]
impl Include {
    /// Creates a new Include and returns its ID
    #[wasm_bindgen]
    pub fn create(addition: String, including_case: String) -> String {
        ...
        let instance = Self {
            ...
            addition: addition,
            including_case: including_case,
        };

        INCLUDE_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Updates a Include instance
    /// Takes a JavaScript object and updates the registry
    #[wasm_bindgen]
    pub fn update(value: JsValue) -> Result<(), JsValue> {
        let instance: Include = serde_wasm_bindgen::from_value(value)
    ...
    /// Checks if a Include exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all Include instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<Include> = INCLUDE_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of Include instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        INCLUDE_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Finds Include instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: Include = serde_wasm_bindgen::from_value(instance_js)
    ...
    /// Sets the addition reference
    #[wasm_bindgen]
    pub fn set_addition(instance_id: String, ref_id: String) -> Result<(), JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Include = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        instance.addition = ref_id;
    ...
    /// Sets the including_case reference
    #[wasm_bindgen]
    pub fn set_including_case(instance_id: String, ref_id: String) -> Result<(), JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Include = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
        ...
        if let Ok(target_js) = UseCase::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<UseCase>(target_js) {
                if !target.include.contains(&instance_id) {
                    target.include.push(instance_id.clone());
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
        ...
        instance.including_case = ref_id;
...
impl Include {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate addition reference exists
        if !UseCase::exists(self.addition.clone()) {
            errors.push(format!("UseCase {} not found", self.addition));
        }

        // Validate including_case reference exists
        if !UseCase::exists(self.including_case.clone()) {
            errors.push(format!("UseCase {} not found", self.including_case));
        }
