...
pub struct TimeObservation {
    ...
    event: Rc<RefCell<NamedElement>>,
    first_event: bool,
}

impl TimeObservation {
    pub fn new(event: Rc<RefCell<NamedElement>>, first_event: bool) -> Self {
        Self {
    ...
    pub fn event(&self) -> &Rc<RefCell<NamedElement>> {
        &self.event
    }

    /// Returns a mutable reference to event
    pub fn event_mut(&mut self) -> &mut Rc<RefCell<NamedElement>> {
        &mut self.event
    }

    /// Sets event
    pub fn set_event(&mut self, value: Rc<RefCell<NamedElement>>) {
        self.event = value;
    }
...
impl Default for TimeObservation {
    fn default() -> Self {
        Self {
            e_annotations: Vec::new(),
            owned_comment: Vec::new(),
...
impl std::fmt::Display for TimeObservation {
