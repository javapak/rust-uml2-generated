...
impl Stereotype {
    ...
    /// Returns a slice of icon
    pub fn icon(&self) -> &[Image] {
        &self.icon
    }

    /// Returns a mutable reference to icon
    pub fn icon_mut(&mut self) -> &mut Vec<Image> {
        &mut self.icon
    }

    /// Adds an item to icon
    pub fn add_icon(&mut self, item: Image) {
        self.icon.push(item);
    }

    /// Clears all items from icon
