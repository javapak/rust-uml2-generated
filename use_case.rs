...
pub struct UseCase {
    ...
    owned_use_case: Vec<Box<UseCase>>,
    use_case: Vec<Weak<RefCell<UseCase>>>,
    ...
    include: Vec<Include>,
    extend: Vec<Extend>,
    extension_point: Vec<ExtensionPoint>,
    subject: Vec<Rc<RefCell<Classifier>>>,
}
...
impl UseCase {
    ...
    /// Returns a reference to owned_use_case
    pub fn owned_use_case(&self) -> &Vec<Box<UseCase>> {
        &self.owned_use_case
    }

    /// Returns a mutable reference to owned_use_case
    pub fn owned_use_case_mut(&mut self) -> &mut Vec<Box<UseCase>> {
        &mut self.owned_use_case
    }

    /// Adds an item to owned_use_case
    pub fn add_owned_use_case(&mut self, item: UseCase) {
        self.owned_use_case.push(Box::new(item));
    }
    ...
    pub fn use_case(&self) -> &Vec<Weak<RefCell<UseCase>>> {
        &self.use_case
    }

    /// Returns a mutable reference to use_case
    pub fn use_case_mut(&mut self) -> &mut Vec<Weak<RefCell<UseCase>>> {
        &mut self.use_case
    }

    /// Adds an item to use_case
    pub fn add_use_case(&mut self, item: Weak<RefCell<UseCase>>) {
    ...
    /// Returns a slice of include
    pub fn include(&self) -> &[Include] {
        &self.include
    }

    /// Returns a mutable reference to include
    pub fn include_mut(&mut self) -> &mut Vec<Include> {
        &mut self.include
    }

    /// Adds an item to include
    pub fn add_include(&mut self, item: Include) {
        self.include.push(item);
    }

    /// Clears all items from include
    pub fn clear_include(&mut self) {
        self.include.clear();
    }

    /// Returns a slice of extend
    pub fn extend(&self) -> &[Extend] {
        &self.extend
    }

    /// Returns a mutable reference to extend
    pub fn extend_mut(&mut self) -> &mut Vec<Extend> {
        &mut self.extend
    }

    /// Adds an item to extend
    pub fn add_extend(&mut self, item: Extend) {
        self.extend.push(item);
    }

    /// Clears all items from extend
    pub fn clear_extend(&mut self) {
        self.extend.clear();
    }

    /// Returns a slice of extension_point
    pub fn extension_point(&self) -> &[ExtensionPoint] {
        &self.extension_point
    }

    /// Returns a mutable reference to extension_point
    pub fn extension_point_mut(&mut self) -> &mut Vec<ExtensionPoint> {
        &mut self.extension_point
    }

    /// Adds an item to extension_point
    pub fn add_extension_point(&mut self, item: ExtensionPoint) {
        self.extension_point.push(item);
    }

    /// Clears all items from extension_point
    pub fn clear_extension_point(&mut self) {
        self.extension_point.clear();
    }

    /// Returns a reference to subject
    pub fn subject(&self) -> &Vec<Rc<RefCell<Classifier>>> {
        &self.subject
    }

    /// Returns a mutable reference to subject
    pub fn subject_mut(&mut self) -> &mut Vec<Rc<RefCell<Classifier>>> {
        &mut self.subject
    }

    /// Adds an item to subject
    pub fn add_subject(&mut self, item: Rc<RefCell<Classifier>>) {
        self.subject.push(item);
    }

    /// Clears all items from subject
