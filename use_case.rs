...
lazy_static! {
    static ref USE_CASE_REGISTRY: Mutex<RefCell<HashMap<String, UseCase>>> = 
...
#[wasm_bindgen]
impl UseCase {
    /// Creates a new UseCase and returns its ID
    #[wasm_bindgen]
    ...
    /// Checks if a UseCase exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all UseCase instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<UseCase> = USE_CASE_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of UseCase instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        USE_CASE_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Finds UseCase instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: UseCase = serde_wasm_bindgen::from_value(instance_js)
    ...
    pub fn add_include(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Ok(target_js) = Include::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Include>(target_js) {
                target.including_case = instance_id;
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = Include::update(target_js);
                }
            }
        }
    ...
    /// Removes a Include from include
    #[wasm_bindgen]
    pub fn remove_include(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(pos) = instance.include.iter().position(|x| x == &ref_id) {
            instance.include.remove(pos);
    ...
    /// Clears all Include from include
    #[wasm_bindgen]
    ...
    pub fn add_extend(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Ok(target_js) = Extend::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Extend>(target_js) {
                target.extension = instance_id;
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = Extend::update(target_js);
                }
            }
        }
    ...
    pub fn remove_extend(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(pos) = instance.extend.iter().position(|x| x == &ref_id) {
            instance.extend.remove(pos);
    ...
    /// Adds a ExtensionPoint to extension_point
    #[wasm_bindgen]
    pub fn add_extension_point(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Ok(target_js) = ExtensionPoint::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<ExtensionPoint>(target_js) {
                target.use_case = instance_id;
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = ExtensionPoint::update(target_js);
                }
            }
        }
    ...
    /// Removes a ExtensionPoint from extension_point
    #[wasm_bindgen]
    pub fn remove_extension_point(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(pos) = instance.extension_point.iter().position(|x| x == &ref_id) {
            instance.extension_point.remove(pos);
    ...
    /// Clears all ExtensionPoint from extension_point
    #[wasm_bindgen]
        ...
        if let Ok(target_js) = Classifier::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Classifier>(target_js) {
                if !target.use_case.contains(&instance_id) {
                    target.use_case.push(instance_id);
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
    ...
    /// Removes a Classifier from subject
    #[wasm_bindgen]
    pub fn remove_subject(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(pos) = instance.subject.iter().position(|x| x == &ref_id) {
            instance.subject.remove(pos);

        if let Ok(target_js) = Classifier::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Classifier>(target_js) {
                target.use_case.retain(|x| x != &instance_id);
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = Classifier::update(target_js);
                }
            }
    ...
    /// Clears all Classifier from subject
    #[wasm_bindgen]
...
impl UseCase {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate all include references exist
        for id in &self.include {
            if !Include::exists(id.clone()) {
                errors.push(format!("Include {} not found", id));
            }
        }

        // Validate all extend references exist
        for id in &self.extend {
            if !Extend::exists(id.clone()) {
                errors.push(format!("Extend {} not found", id));
            }
        }

        // Validate all extension_point references exist
        for id in &self.extension_point {
            if !ExtensionPoint::exists(id.clone()) {
                errors.push(format!("ExtensionPoint {} not found", id));
            }
        }

        // Validate all subject references exist
        for id in &self.subject {
            if !Classifier::exists(id.clone()) {
                errors.push(format!("Classifier {} not found", id));
            }
        }
