...
lazy_static! {
    static ref OPERATION_REGISTRY: Mutex<RefCell<HashMap<String, Operation>>> = 
...
impl Operation {
    ...
    pub fn create(is_leaf: String, is_static: String, is_abstract: String, concurrency: CallConcurrencyKind, is_query: String) -> String {
        ...
        OPERATION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Checks if a Operation exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all Operation instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<Operation> = OPERATION_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of Operation instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        OPERATION_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all Operation instances
    #[wasm_bindgen]
    pub fn clear_all() {
        OPERATION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds Operation instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: Operation = serde_wasm_bindgen::from_value(instance_js)
    ...
    pub fn set_interface(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        ...
        if let Some(old_ref_id) = &instance.interface {
            if let Ok(target_js) = Interface::get(old_ref_id.clone()) {
                if let Ok(mut target) = serde_wasm_bindgen::from_value::<Interface>(target_js) {
                    target.owned_operation.retain(|x| x != &instance_id);
                    if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                        let _ = Interface::update(target_js);
                    }
                }
            }
        }
        ...
        // Add to new opposite
        if let Some(new_ref_id) = &ref_id {
        if let Ok(target_js) = Interface::get(new_ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Interface>(target_js) {
                if !target.owned_operation.contains(&instance_id) {
                    target.owned_operation.push(instance_id.clone());
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = Interface::update(target_js);
                }
    ...
    pub fn set_class(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        ...
        if let Some(old_ref_id) = &instance.class {
            if let Ok(target_js) = Class::get(old_ref_id.clone()) {
                if let Ok(mut target) = serde_wasm_bindgen::from_value::<Class>(target_js) {
                    target.owned_operation.retain(|x| x != &instance_id);
                    if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                        let _ = Class::update(target_js);
                    }
                }
            }
        }
        ...
        // Add to new opposite
        if let Some(new_ref_id) = &ref_id {
        if let Ok(target_js) = Class::get(new_ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Class>(target_js) {
                if !target.owned_operation.contains(&instance_id) {
                    target.owned_operation.push(instance_id.clone());
                }
    ...
    /// Adds a Operation to redefined_operation
    #[wasm_bindgen]
    ...
    /// Removes a Operation from redefined_operation
    #[wasm_bindgen]
    pub fn remove_redefined_operation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(pos) = instance.redefined_operation.iter().position(|x| x == &ref_id) {
            instance.redefined_operation.remove(pos);
    ...
    /// Clears all Operation from redefined_operation
    #[wasm_bindgen]
    ...
    /// Sets the body_condition reference
    #[wasm_bindgen]
    pub fn set_body_condition(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        ...
        instance.body_condition = ref_id;
...
impl Operation {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate interface reference exists
        ...
        if let Some(id) = &self.class {
            if !Class::exists(id.clone()) {
                errors.push(format!("Class {} not found", id));
            }
        }
        ...
        // Validate all redefined_operation references exist
        for id in &self.redefined_operation {
        ...
        // Validate body_condition reference exists
        if let Some(id) = &self.body_condition {
            if !Constraint::exists(id.clone()) {
                errors.push(format!("Constraint {} not found", id));
            }
        }
