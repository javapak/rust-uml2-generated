...
lazy_static! {
    static ref PORT_REGISTRY: Mutex<RefCell<HashMap<String, Port>>> = 
...
impl Port {
    ...
    /// Gets all Port instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<Port> = PORT_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of Port instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        PORT_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Finds Port instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: Port = serde_wasm_bindgen::from_value(instance_js)
    ...
    /// Removes a Port from redefined_port
    #[wasm_bindgen]
    pub fn remove_redefined_port(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(pos) = instance.redefined_port.iter().position(|x| x == &ref_id) {
            instance.redefined_port.remove(pos);
    ...
    /// Clears all Port from redefined_port
    #[wasm_bindgen]
...
impl Port {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate all redefined_port references exist
        for id in &self.redefined_port {
