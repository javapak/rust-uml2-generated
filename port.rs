...
pub struct Port {
    ...
    is_behavior: bool,
    is_service: bool,
    redefined_port: Vec<Rc<RefCell<Port>>>,
    protocol: Option<Rc<RefCell<ProtocolStateMachine>>>,
}

impl Port {
    pub fn new(is_leaf: bool, is_static: bool, is_ordered: bool, is_unique: bool, is_read_only: bool, is_derived: bool, is_derived_union: bool, aggregation: String, is_behavior: bool, is_service: bool) -> Self {
        Self {
    ...
    /// Sets is_behavior
    pub fn set_is_behavior(&mut self, value: bool) {
        self.is_behavior = value;
    }
    ...
    /// Sets is_service
    pub fn set_is_service(&mut self, value: bool) {
        self.is_service = value;
    }

    /// Returns a reference to redefined_port
    pub fn redefined_port(&self) -> &Vec<Rc<RefCell<Port>>> {
        &self.redefined_port
    }

    /// Returns a mutable reference to redefined_port
    pub fn redefined_port_mut(&mut self) -> &mut Vec<Rc<RefCell<Port>>> {
        &mut self.redefined_port
    }

    /// Adds an item to redefined_port
    pub fn add_redefined_port(&mut self, item: Rc<RefCell<Port>>) {
        self.redefined_port.push(item);
    }

    /// Clears all items from redefined_port
    pub fn clear_redefined_port(&mut self) {
        self.redefined_port.clear();
    }

    /// Returns a reference to protocol if present
    pub fn protocol(&self) -> Option<&Rc<RefCell<ProtocolStateMachine>>> {
        self.protocol.as_ref()
    }

    /// Returns a mutable reference to protocol if present
    pub fn protocol_mut(&mut self) -> Option<&mut Rc<RefCell<ProtocolStateMachine>>> {
        self.protocol.as_mut()
    }

    /// Sets protocol
    pub fn set_protocol(&mut self, value: Rc<RefCell<ProtocolStateMachine>>) {
        self.protocol = Some(value);
    }

    /// Takes protocol, leaving None in its place
    pub fn take_protocol(&mut self) -> Option<Rc<RefCell<ProtocolStateMachine>>> {
        self.protocol.take()
    }
