...
lazy_static! {
    static ref STATE_REGISTRY: Mutex<RefCell<HashMap<String, State>>> = 
...
impl State {
    ...
    /// Gets all State instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<State> = STATE_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of State instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        STATE_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Finds State instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: State = serde_wasm_bindgen::from_value(instance_js)
