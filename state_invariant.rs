...
impl StateInvariant {
    pub fn new(invariant: Constraint) -> Self {
        Self {
    ...
    /// Returns a reference to invariant
    pub fn invariant(&self) -> &Constraint {
        &self.invariant
    }

    /// Returns a mutable reference to invariant
    pub fn invariant_mut(&mut self) -> &mut Constraint {
        &mut self.invariant
    }

    /// Sets invariant
    pub fn set_invariant(&mut self, value: Constraint) {
        self.invariant = value;
    }

}

impl Default for StateInvariant {
    fn default() -> Self {
        Self {
            e_annotations: Vec::new(),
            owned_comment: Vec::new(),
...
impl std::fmt::Display for StateInvariant {
