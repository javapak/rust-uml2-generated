...
pub struct BehaviorExecutionSpecification {
    ...
    behavior: Option<Rc<RefCell<Behavior>>>,
}

impl BehaviorExecutionSpecification {
    pub fn new(start: Rc<RefCell<OccurrenceSpecification>>, finish: Rc<RefCell<OccurrenceSpecification>>) -> Self {
        Self {
    ...
    /// Returns a reference to behavior if present
    pub fn behavior(&self) -> Option<&Rc<RefCell<Behavior>>> {
        self.behavior.as_ref()
    }

    /// Returns a mutable reference to behavior if present
    pub fn behavior_mut(&mut self) -> Option<&mut Rc<RefCell<Behavior>>> {
    ...
    /// Takes behavior, leaving None in its place
    pub fn take_behavior(&mut self) -> Option<Rc<RefCell<Behavior>>> {
        self.behavior.take()
    }

}

impl Default for BehaviorExecutionSpecification {
    fn default() -> Self {
        Self {
            e_annotations: Vec::new(),
            owned_comment: Vec::new(),
...
impl std::fmt::Display for BehaviorExecutionSpecification {
