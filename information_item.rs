...
pub struct InformationItem {
    ...
    represented: Vec<Rc<RefCell<Classifier>>>,
}
...
impl InformationItem {
    ...
    /// Returns a reference to represented
    pub fn represented(&self) -> &Vec<Rc<RefCell<Classifier>>> {
        &self.represented
    }

    /// Returns a mutable reference to represented
    pub fn represented_mut(&mut self) -> &mut Vec<Rc<RefCell<Classifier>>> {
        &mut self.represented
    }

    /// Adds an item to represented
    pub fn add_represented(&mut self, item: Rc<RefCell<Classifier>>) {
        self.represented.push(item);
    }

    /// Clears all items from represented
    pub fn clear_represented(&mut self) {
        self.represented.clear();
    }

}

impl std::fmt::Display for InformationItem {
