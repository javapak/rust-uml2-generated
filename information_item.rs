...
lazy_static! {
    static ref INFORMATION_ITEM_REGISTRY: Mutex<RefCell<HashMap<String, InformationItem>>> = 
...
impl InformationItem {
    /// Creates a new InformationItem and returns its ID
    #[wasm_bindgen]
    pub fn create(is_leaf: String, is_abstract: String) -> String {
        ...
        INFORMATION_ITEM_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Updates a InformationItem instance
    /// Takes a JavaScript object and updates the registry
    #[wasm_bindgen]
    pub fn update(value: JsValue) -> Result<(), JsValue> {
        let instance: InformationItem = serde_wasm_bindgen::from_value(value)
    ...
    /// Deletes a InformationItem by ID
    ...
    /// Checks if a InformationItem exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all InformationItem instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<InformationItem> = INFORMATION_ITEM_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of InformationItem instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        INFORMATION_ITEM_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all InformationItem instances
    #[wasm_bindgen]
    pub fn clear_all() {
        INFORMATION_ITEM_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds InformationItem instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: InformationItem = serde_wasm_bindgen::from_value(instance_js)
    ...
    /// Adds a Classifier to represented
    #[wasm_bindgen]
    pub fn add_represented(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: InformationItem = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if instance.represented.contains(&ref_id) {
            return Ok(false);
        }

        instance.represented.push(ref_id.clone());
    ...
    /// Removes a Classifier from represented
    #[wasm_bindgen]
    pub fn remove_represented(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: InformationItem = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if let Some(pos) = instance.represented.iter().position(|x| x == &ref_id) {
            instance.represented.remove(pos);
    ...
    /// Clears all Classifier from represented
    #[wasm_bindgen]
    pub fn clear_represented(instance_id: String) -> Result<usize, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: InformationItem = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        let count = instance.represented.len();

        instance.represented.clear();
...
impl InformationItem {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate all represented references exist
        for id in &self.represented {
            if !Classifier::exists(id.clone()) {
                errors.push(format!("Classifier {} not found", id));
            }
        }
