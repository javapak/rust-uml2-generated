...
pub struct TimeExpression {
    ...
    expr: Option<Rc<RefCell<ValueSpecification>>>,
    observation: Vec<Rc<RefCell<Observation>>>,
}
...
impl TimeExpression {
    ...
    /// Returns a reference to expr if present
    pub fn expr(&self) -> Option<&Rc<RefCell<ValueSpecification>>> {
        self.expr.as_ref()
    }

    /// Returns a mutable reference to expr if present
    pub fn expr_mut(&mut self) -> Option<&mut Rc<RefCell<ValueSpecification>>> {
        self.expr.as_mut()
    }

    /// Sets expr
    pub fn set_expr(&mut self, value: Rc<RefCell<ValueSpecification>>) {
        self.expr = Some(value);
    }

    /// Takes expr, leaving None in its place
    pub fn take_expr(&mut self) -> Option<Rc<RefCell<ValueSpecification>>> {
        self.expr.take()
    }

    /// Returns a reference to observation
    pub fn observation(&self) -> &Vec<Rc<RefCell<Observation>>> {
        &self.observation
    }

    /// Returns a mutable reference to observation
    pub fn observation_mut(&mut self) -> &mut Vec<Rc<RefCell<Observation>>> {
        &mut self.observation
    }

    /// Adds an item to observation
    pub fn add_observation(&mut self, item: Rc<RefCell<Observation>>) {
        self.observation.push(item);
    }

    /// Clears all items from observation
...
impl std::fmt::Display for TimeExpression {
