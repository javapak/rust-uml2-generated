...
lazy_static! {
    static ref COMPONENT_REALIZATION_REGISTRY: Mutex<RefCell<HashMap<String, ComponentRealization>>> = 
...
pub struct ComponentRealization {
    ...
    pub realizing_classifier: String,
}

#[wasm_bindgen]
impl ComponentRealization {
    /// Creates a new ComponentRealization and returns its ID
    #[wasm_bindgen]
    pub fn create(realizing_classifier: String) -> String {
        ...
        COMPONENT_REALIZATION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Gets a ComponentRealization by ID
    ...
    /// Updates a ComponentRealization instance
    /// Takes a JavaScript object and updates the registry
    #[wasm_bindgen]
    pub fn update(value: JsValue) -> Result<(), JsValue> {
        let instance: ComponentRealization = serde_wasm_bindgen::from_value(value)
    ...
    /// Deletes a ComponentRealization by ID
    ...
    /// Checks if a ComponentRealization exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all ComponentRealization instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<ComponentRealization> = COMPONENT_REALIZATION_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of ComponentRealization instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        COMPONENT_REALIZATION_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all ComponentRealization instances
    #[wasm_bindgen]
    pub fn clear_all() {
        COMPONENT_REALIZATION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds ComponentRealization instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: ComponentRealization = serde_wasm_bindgen::from_value(instance_js)
    ...
    /// Sets the abstraction reference
    #[wasm_bindgen]
    pub fn set_abstraction(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ComponentRealization = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        // Remove from old opposite
        if let Some(old_ref_id) = &instance.abstraction {
            if let Ok(target_js) = Component::get(old_ref_id.clone()) {
                if let Ok(mut target) = serde_wasm_bindgen::from_value::<Component>(target_js) {
                    target.realization.retain(|x| x != &instance_id);
                    if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                        let _ = Component::update(target_js);
                    }
                }
            }
        }
        ...
        // Add to new opposite
        if let Some(new_ref_id) = &ref_id {
        if let Ok(target_js) = Component::get(new_ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Component>(target_js) {
                if !target.realization.contains(&instance_id) {
                    target.realization.push(instance_id.clone());
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = Component::update(target_js);
                }
    ...
    /// Sets the realizing_classifier reference
    #[wasm_bindgen]
    pub fn set_realizing_classifier(instance_id: String, ref_id: String) -> Result<(), JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ComponentRealization = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        instance.realizing_classifier = ref_id;
...
impl ComponentRealization {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate abstraction reference exists
        if let Some(id) = &self.abstraction {
            if !Component::exists(id.clone()) {
                errors.push(format!("Component {} not found", id));
            }
        }

        // Validate realizing_classifier reference exists
        if !Classifier::exists(self.realizing_classifier.clone()) {
            errors.push(format!("Classifier {} not found", self.realizing_classifier));
        }
