...
pub struct ComponentRealization {
    ...
    abstraction: Option<Weak<RefCell<Component>>>,
    realizing_classifier: Rc<RefCell<Classifier>>,
}

impl ComponentRealization {
    pub fn new(supplier: Vec<Rc<RefCell<NamedElement>>>, client: Vec<Rc<RefCell<NamedElement>>>, realizing_classifier: Rc<RefCell<Classifier>>) -> Self {
        Self {
    ...
    /// Returns a reference to abstraction if present
    pub fn abstraction(&self) -> Option<&Weak<RefCell<Component>>> {
        self.abstraction.as_ref()
    }

    /// Returns a mutable reference to abstraction if present
    pub fn abstraction_mut(&mut self) -> Option<&mut Weak<RefCell<Component>>> {
        self.abstraction.as_mut()
    }

    /// Sets abstraction
    pub fn set_abstraction(&mut self, value: Weak<RefCell<Component>>) {
        self.abstraction = Some(value);
    }

    /// Takes abstraction, leaving None in its place
    pub fn take_abstraction(&mut self) -> Option<Weak<RefCell<Component>>> {
        self.abstraction.take()
    }

    /// Returns a reference to realizing_classifier
    pub fn realizing_classifier(&self) -> &Rc<RefCell<Classifier>> {
        &self.realizing_classifier
    }

    /// Returns a mutable reference to realizing_classifier
    pub fn realizing_classifier_mut(&mut self) -> &mut Rc<RefCell<Classifier>> {
        &mut self.realizing_classifier
    }

    /// Sets realizing_classifier
    pub fn set_realizing_classifier(&mut self, value: Rc<RefCell<Classifier>>) {
