...
impl OpaqueAction {
    ...
    /// Returns a slice of output_value
    pub fn output_value(&self) -> &[OutputPin] {
        &self.output_value
    }

    /// Returns a mutable reference to output_value
    pub fn output_value_mut(&mut self) -> &mut Vec<OutputPin> {
        &mut self.output_value
    }

    /// Adds an item to output_value
    pub fn add_output_value(&mut self, item: OutputPin) {
        self.output_value.push(item);
    }

    /// Clears all items from output_value
...
impl std::fmt::Display for OpaqueAction {
