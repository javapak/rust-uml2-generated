...
lazy_static! {
    static ref ACTIVITY_REGISTRY: Mutex<RefCell<HashMap<String, Activity>>> = 
...
impl Activity {
    ...
    /// Checks if a Activity exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all Activity instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<Activity> = ACTIVITY_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of Activity instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        ACTIVITY_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all Activity instances
    #[wasm_bindgen]
    pub fn clear_all() {
        ACTIVITY_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds Activity instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: Activity = serde_wasm_bindgen::from_value(instance_js)
    ...
    pub fn add_variable(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Ok(target_js) = Variable::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Variable>(target_js) {
                target.activity_scope = Some(instance_id);
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
    ...
    pub fn add_node(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Ok(target_js) = ActivityNode::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<ActivityNode>(target_js) {
                target.activity = Some(instance_id);
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
    ...
    /// Adds a ActivityPartition to partition
    #[wasm_bindgen]
    ...
    /// Removes a ActivityPartition from partition
    #[wasm_bindgen]
    pub fn remove_partition(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(pos) = instance.partition.iter().position(|x| x == &ref_id) {
            instance.partition.remove(pos);
    ...
    /// Clears all ActivityPartition from partition
    #[wasm_bindgen]
    ...
    pub fn add_group(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Ok(target_js) = ActivityGroup::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<ActivityGroup>(target_js) {
                target.in_activity = Some(instance_id);
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = ActivityGroup::update(target_js);
                }
            }
        }
    ...
    /// Removes a ActivityGroup from group
    #[wasm_bindgen]
    pub fn remove_group(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(pos) = instance.group.iter().position(|x| x == &ref_id) {
            instance.group.remove(pos);
    ...
    /// Clears all ActivityGroup from group
    #[wasm_bindgen]
    ...
    /// Sets the is_single_execution field
    #[wasm_bindgen]
...
impl Activity {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        for id in &self.group {
            if !ActivityGroup::exists(id.clone()) {
                errors.push(format!("ActivityGroup {} not found", id));
            }
        }
