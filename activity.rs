...
pub struct Activity {
    ...
    partition: Vec<Rc<RefCell<ActivityPartition>>>,
...
impl Activity {
    ...
    /// Returns a reference to partition
    pub fn partition(&self) -> &Vec<Rc<RefCell<ActivityPartition>>> {
        &self.partition
    }

    /// Returns a mutable reference to partition
    pub fn partition_mut(&mut self) -> &mut Vec<Rc<RefCell<ActivityPartition>>> {
        &mut self.partition
    }

    /// Adds an item to partition
    pub fn add_partition(&mut self, item: Rc<RefCell<ActivityPartition>>) {
        self.partition.push(item);
    }
    ...
    /// Returns a slice of group
    pub fn group(&self) -> &[ActivityGroup] {
        &self.group
    }

    /// Returns a mutable reference to group
    pub fn group_mut(&mut self) -> &mut Vec<ActivityGroup> {
        &mut self.group
    }

    /// Adds an item to group
    pub fn add_group(&mut self, item: ActivityGroup) {
        self.group.push(item);
    }

    /// Clears all items from group
