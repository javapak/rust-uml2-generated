...
lazy_static! {
    static ref SUBSTITUTION_REGISTRY: Mutex<RefCell<HashMap<String, Substitution>>> = 
...
#[wasm_bindgen]
impl Substitution {
    /// Creates a new Substitution and returns its ID
    #[wasm_bindgen]
    pub fn create(contract: String, substituting_classifier: String) -> String {
        ...
        let instance = Self {
            ...
            mapping: None,
            contract: contract,
            substituting_classifier: substituting_classifier,
        };

        SUBSTITUTION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Updates a Substitution instance
    ...
    /// Checks if a Substitution exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all Substitution instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<Substitution> = SUBSTITUTION_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of Substitution instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        SUBSTITUTION_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all Substitution instances
    #[wasm_bindgen]
    pub fn clear_all() {
        SUBSTITUTION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds Substitution instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: Substitution = serde_wasm_bindgen::from_value(instance_js)
    ...
    /// Sets the substituting_classifier reference
    #[wasm_bindgen]
    pub fn set_substituting_classifier(instance_id: String, ref_id: String) -> Result<(), JsValue> {
        ...
        if let Ok(target_js) = Classifier::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Classifier>(target_js) {
                if !target.substitution.contains(&instance_id) {
                    target.substitution.push(instance_id.clone());
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
        ...
        instance.substituting_classifier = ref_id;
...
impl Substitution {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        if !Classifier::exists(self.contract.clone()) {
            errors.push(format!("Classifier {} not found", self.contract));
        }

        // Validate substituting_classifier reference exists
        if !Classifier::exists(self.substituting_classifier.clone()) {
            errors.push(format!("Classifier {} not found", self.substituting_classifier));
        }
