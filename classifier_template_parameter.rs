...
pub struct ClassifierTemplateParameter {
    ...
    default_classifier: Option<Rc<RefCell<Classifier>>>,
    constraining_classifier: Option<Rc<RefCell<Classifier>>>,
}

impl ClassifierTemplateParameter {
    pub fn new(signature: Weak<RefCell<TemplateSignature>>, parametered_element: Rc<RefCell<ParameterableElement>>, allow_substitutable: bool) -> Self {
        Self {
    ...
    /// Returns a reference to default_classifier if present
    pub fn default_classifier(&self) -> Option<&Rc<RefCell<Classifier>>> {
        self.default_classifier.as_ref()
    }

    /// Returns a mutable reference to default_classifier if present
    pub fn default_classifier_mut(&mut self) -> Option<&mut Rc<RefCell<Classifier>>> {
        self.default_classifier.as_mut()
    }

    /// Sets default_classifier
    pub fn set_default_classifier(&mut self, value: Rc<RefCell<Classifier>>) {
        self.default_classifier = Some(value);
    }

    /// Takes default_classifier, leaving None in its place
    pub fn take_default_classifier(&mut self) -> Option<Rc<RefCell<Classifier>>> {
        self.default_classifier.take()
    }

    /// Returns a reference to constraining_classifier if present
    pub fn constraining_classifier(&self) -> Option<&Rc<RefCell<Classifier>>> {
        self.constraining_classifier.as_ref()
    }

    /// Returns a mutable reference to constraining_classifier if present
    pub fn constraining_classifier_mut(&mut self) -> Option<&mut Rc<RefCell<Classifier>>> {
        self.constraining_classifier.as_mut()
    }

    /// Sets constraining_classifier
    pub fn set_constraining_classifier(&mut self, value: Rc<RefCell<Classifier>>) {
        self.constraining_classifier = Some(value);
    }

    /// Takes constraining_classifier, leaving None in its place
    pub fn take_constraining_classifier(&mut self) -> Option<Rc<RefCell<Classifier>>> {
