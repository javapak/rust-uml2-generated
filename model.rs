...
impl Model {
    ...
    /// Returns a reference to viewpoint if present
    pub fn viewpoint(&self) -> Option<&String> {
        self.viewpoint.as_ref()
    }

    /// Returns a mutable reference to viewpoint if present
    pub fn viewpoint_mut(&mut self) -> Option<&mut String> {
        self.viewpoint.as_mut()
    }
