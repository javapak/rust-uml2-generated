...
lazy_static! {
    static ref SIGNAL_REGISTRY: Mutex<RefCell<HashMap<String, Signal>>> = 
...
#[wasm_bindgen]
impl Signal {
    /// Creates a new Signal and returns its ID
    #[wasm_bindgen]
    ...
    /// Updates a Signal instance
    /// Takes a JavaScript object and updates the registry
    #[wasm_bindgen]
    pub fn update(value: JsValue) -> Result<(), JsValue> {
        let instance: Signal = serde_wasm_bindgen::from_value(value)
    ...
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<Signal> = SIGNAL_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of Signal instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        SIGNAL_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Finds Signal instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: Signal = serde_wasm_bindgen::from_value(instance_js)
