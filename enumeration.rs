...
use crate::enumeration_literal::EnumerationLiteral;

lazy_static! {
    static ref ENUMERATION_REGISTRY: Mutex<RefCell<HashMap<String, Enumeration>>> = 
...
pub struct Enumeration {
    ...
    pub owned_literal: Vec<String>,
}

#[wasm_bindgen]
impl Enumeration {
    /// Creates a new Enumeration and returns its ID
    #[wasm_bindgen]
    pub fn create(is_leaf: String, is_abstract: String) -> String {
        ...
        let instance = Self {
            ...
            owned_literal: Vec::new(),
        };

        ENUMERATION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Updates a Enumeration instance
    /// Takes a JavaScript object and updates the registry
    #[wasm_bindgen]
    pub fn update(value: JsValue) -> Result<(), JsValue> {
        let instance: Enumeration = serde_wasm_bindgen::from_value(value)
    ...
    /// Checks if a Enumeration exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all Enumeration instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<Enumeration> = ENUMERATION_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of Enumeration instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        ENUMERATION_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all Enumeration instances
    #[wasm_bindgen]
    pub fn clear_all() {
        ENUMERATION_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds Enumeration instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: Enumeration = serde_wasm_bindgen::from_value(instance_js)
    ...
    pub fn add_owned_attribute(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Ok(target_js) = Property::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Property>(target_js) {
                target.datatype = Some(instance_id);
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
    ...
    pub fn clear_owned_attribute(instance_id: String) -> Result<usize, JsValue> {
        ...
        // Update all opposite references
        for ref_id in &instance.owned_attribute {
            if let Ok(target_js) = Property::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Property>(target_js) {
                target.datatype = None;
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
    ...
    /// Adds a EnumerationLiteral to owned_literal
    #[wasm_bindgen]
    pub fn add_owned_literal(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Enumeration = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if instance.owned_literal.contains(&ref_id) {
            return Ok(false);
        }

        instance.owned_literal.push(ref_id.clone());

        if let Ok(target_js) = EnumerationLiteral::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<EnumerationLiteral>(target_js) {
                target.enumeration = Some(instance_id);
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = EnumerationLiteral::update(target_js);
                }
            }
        }
    ...
    /// Removes a EnumerationLiteral from owned_literal
    #[wasm_bindgen]
    pub fn remove_owned_literal(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Enumeration = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if let Some(pos) = instance.owned_literal.iter().position(|x| x == &ref_id) {
            instance.owned_literal.remove(pos);

        if let Ok(target_js) = EnumerationLiteral::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<EnumerationLiteral>(target_js) {
                target.enumeration = None;
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = EnumerationLiteral::update(target_js);
                }
            }
    ...
    /// Clears all EnumerationLiteral from owned_literal
    #[wasm_bindgen]
    pub fn clear_owned_literal(instance_id: String) -> Result<usize, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Enumeration = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        let count = instance.owned_literal.len();

        // Update all opposite references
        for ref_id in &instance.owned_literal {
            if let Ok(target_js) = EnumerationLiteral::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<EnumerationLiteral>(target_js) {
                target.enumeration = None;
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
...
impl Enumeration {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate all owned_literal references exist
        for id in &self.owned_literal {
            if !EnumerationLiteral::exists(id.clone()) {
                errors.push(format!("EnumerationLiteral {} not found", id));
            }
        }
