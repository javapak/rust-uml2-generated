// NamedElement - Generated UML Class
...
const TYPE_NAME: &str = "NamedElement";
...
pub fn clear_visibility(instance_id: String) -> Result<(), JsValue> {
    ...
    pub fn add_client_dependency(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(mut target) = registry::get::<Dependency>(&ref_id) {
            if !target.client.contains(&instance_id) {
                target.client.push(instance_id.clone());
            }
            let _ = registry::insert(ref_id.clone(), "Dependency", &target);
        }
    ...
    pub fn remove_client_dependency(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let initial_len = instance.client_dependency.len();
        instance.client_dependency.retain(|id| id != &ref_id);
        let removed = instance.client_dependency.len() < initial_len;
        ...
        // Update opposite reference
        if let Some(mut target) = registry::get::<Dependency>(&ref_id) {
            target.client.retain(|id| id != &instance_id);
            let _ = registry::insert(ref_id.clone(), "Dependency", &target);
        }
    ...
    pub fn set_name_expression(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        ...
        instance.name_expression = ref_id.clone();
    ...
    #[wasm_bindgen]
    pub fn clear_name_expression(instance_id: String) -> Result<bool, JsValue> {
        let mut instance: Self = registry::get(&instance_id)
            .ok_or_else(|| JsValue::from_str("Instance not found"))?;
        
        if instance.name_expression.is_none() {
            return Ok(false);
        }
        
        instance.name_expression = None;
