...
lazy_static! {
    static ref EXECUTION_ENVIRONMENT_REGISTRY: Mutex<RefCell<HashMap<String, ExecutionEnvironment>>> = 
...
pub struct ExecutionEnvironment {
    ...
    pub nested_node: Vec<String>,
}

#[wasm_bindgen]
impl ExecutionEnvironment {
    /// Creates a new ExecutionEnvironment and returns its ID
    #[wasm_bindgen]
    pub fn create(is_leaf: String, is_abstract: String, is_active: String) -> String {
        ...
        EXECUTION_ENVIRONMENT_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Gets a ExecutionEnvironment by ID
    ...
    /// Updates a ExecutionEnvironment instance
    /// Takes a JavaScript object and updates the registry
    #[wasm_bindgen]
    pub fn update(value: JsValue) -> Result<(), JsValue> {
        let instance: ExecutionEnvironment = serde_wasm_bindgen::from_value(value)
    ...
    /// Deletes a ExecutionEnvironment by ID
    ...
    /// Checks if a ExecutionEnvironment exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all ExecutionEnvironment instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<ExecutionEnvironment> = EXECUTION_ENVIRONMENT_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of ExecutionEnvironment instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        EXECUTION_ENVIRONMENT_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all ExecutionEnvironment instances
    #[wasm_bindgen]
    pub fn clear_all() {
        EXECUTION_ENVIRONMENT_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds ExecutionEnvironment instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: ExecutionEnvironment = serde_wasm_bindgen::from_value(instance_js)
    ...
    /// Adds a Node to nested_node
    #[wasm_bindgen]
    pub fn add_nested_node(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ExecutionEnvironment = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if instance.nested_node.contains(&ref_id) {
            return Ok(false);
        }

        instance.nested_node.push(ref_id.clone());
    ...
    /// Removes a Node from nested_node
    #[wasm_bindgen]
    pub fn remove_nested_node(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ExecutionEnvironment = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if let Some(pos) = instance.nested_node.iter().position(|x| x == &ref_id) {
            instance.nested_node.remove(pos);
    ...
    /// Clears all Node from nested_node
    #[wasm_bindgen]
    pub fn clear_nested_node(instance_id: String) -> Result<usize, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ExecutionEnvironment = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        let count = instance.nested_node.len();

        instance.nested_node.clear();
...
impl ExecutionEnvironment {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate all nested_node references exist
        for id in &self.nested_node {
            if !Node::exists(id.clone()) {
                errors.push(format!("Node {} not found", id));
            }
        }
