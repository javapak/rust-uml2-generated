...
lazy_static! {
    static ref EXPANSION_NODE_REGISTRY: Mutex<RefCell<HashMap<String, ExpansionNode>>> = 
...
#[wasm_bindgen]
impl ExpansionNode {
    /// Creates a new ExpansionNode and returns its ID
    #[wasm_bindgen]
    pub fn create(is_leaf: String, ordering: ObjectNodeOrderingKind, is_control_type: String, upper_bound: String) -> String {
        ...
        EXPANSION_NODE_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Updates a ExpansionNode instance
    ...
    /// Checks if a ExpansionNode exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all ExpansionNode instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<ExpansionNode> = EXPANSION_NODE_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of ExpansionNode instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        EXPANSION_NODE_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all ExpansionNode instances
    #[wasm_bindgen]
    pub fn clear_all() {
        EXPANSION_NODE_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds ExpansionNode instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: ExpansionNode = serde_wasm_bindgen::from_value(instance_js)
    ...
    /// Sets the upper_bound reference
    #[wasm_bindgen]
    pub fn set_upper_bound(instance_id: String, ref_id: String) -> Result<(), JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ExpansionNode = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        instance.upper_bound = ref_id;
    ...
    /// Adds a State to in_state
    #[wasm_bindgen]
    pub fn add_in_state(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ExpansionNode = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if instance.in_state.contains(&ref_id) {
            return Ok(false);
        }

        instance.in_state.push(ref_id.clone());
    ...
    /// Removes a State from in_state
    #[wasm_bindgen]
    pub fn remove_in_state(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ExpansionNode = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if let Some(pos) = instance.in_state.iter().position(|x| x == &ref_id) {
            instance.in_state.remove(pos);
    ...
    /// Clears all State from in_state
    #[wasm_bindgen]
    pub fn clear_in_state(instance_id: String) -> Result<usize, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: ExpansionNode = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        let count = instance.in_state.len();

        instance.in_state.clear();
    ...
    /// Sets the region_as_output reference
    #[wasm_bindgen]
    pub fn set_region_as_output(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        ...
        if let Some(old_ref_id) = &instance.region_as_output {
            if let Ok(target_js) = ExpansionRegion::get(old_ref_id.clone()) {
                if let Ok(mut target) = serde_wasm_bindgen::from_value::<ExpansionRegion>(target_js) {
                    target.output_element.retain(|x| x != &instance_id);
                    if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                        let _ = ExpansionRegion::update(target_js);
                    }
                }
            }
        }
        ...
        // Add to new opposite
        if let Some(new_ref_id) = &ref_id {
        if let Ok(target_js) = ExpansionRegion::get(new_ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<ExpansionRegion>(target_js) {
                if !target.output_element.contains(&instance_id) {
                    target.output_element.push(instance_id.clone());
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = ExpansionRegion::update(target_js);
                }
        ...
        instance.region_as_output = ref_id;
    ...
    /// Sets the region_as_input reference
    #[wasm_bindgen]
    pub fn set_region_as_input(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        ...
        if let Some(old_ref_id) = &instance.region_as_input {
            if let Ok(target_js) = ExpansionRegion::get(old_ref_id.clone()) {
                if let Ok(mut target) = serde_wasm_bindgen::from_value::<ExpansionRegion>(target_js) {
                    target.input_element.retain(|x| x != &instance_id);
                    if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                        let _ = ExpansionRegion::update(target_js);
                    }
                }
        ...
        if let Ok(target_js) = ExpansionRegion::get(new_ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<ExpansionRegion>(target_js) {
                if !target.input_element.contains(&instance_id) {
                    target.input_element.push(instance_id.clone());
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = ExpansionRegion::update(target_js);
                }
        ...
        instance.region_as_input = ref_id;
    ...
    /// Sets the is_control_type field
    #[wasm_bindgen]
...
impl ExpansionNode {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate upper_bound reference exists
        if !ValueSpecification::exists(self.upper_bound.clone()) {
            errors.push(format!("ValueSpecification {} not found", self.upper_bound));
        }

        // Validate all in_state references exist
        for id in &self.in_state {
        ...
        // Validate region_as_output reference exists
        if let Some(id) = &self.region_as_output {
            if !ExpansionRegion::exists(id.clone()) {
                errors.push(format!("ExpansionRegion {} not found", id));
            }
        }

        // Validate region_as_input reference exists
        if let Some(id) = &self.region_as_input {
            if !ExpansionRegion::exists(id.clone()) {
                errors.push(format!("ExpansionRegion {} not found", id));
            }
        }
