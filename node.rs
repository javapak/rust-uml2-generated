...
impl Node {
    ...
    /// Returns a reference to nested_node
    pub fn nested_node(&self) -> &Vec<Box<Node>> {
        &self.nested_node
    }

    /// Returns a mutable reference to nested_node
    pub fn nested_node_mut(&mut self) -> &mut Vec<Box<Node>> {
        &mut self.nested_node
    }

    /// Adds an item to nested_node
    pub fn add_nested_node(&mut self, item: Node) {
        self.nested_node.push(Box::new(item));
    }

    /// Clears all items from nested_node
    pub fn clear_nested_node(&mut self) {
        self.nested_node.clear();
    }
