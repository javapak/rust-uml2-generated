...
lazy_static! {
    static ref INTERFACE_REGISTRY: Mutex<RefCell<HashMap<String, Interface>>> = 
...
pub struct Interface {
    ...
    pub redefined_interface: Vec<String>,
...
#[wasm_bindgen]
impl Interface {
    /// Creates a new Interface and returns its ID
    #[wasm_bindgen]
    pub fn create(is_leaf: String, is_abstract: String) -> String {
        ...
        let instance = Self {
            ...
            redefined_interface: Vec::new(),
            owned_reception: Vec::new(),
            protocol: None,
        };

        INTERFACE_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Updates a Interface instance
    /// Takes a JavaScript object and updates the registry
    #[wasm_bindgen]
    pub fn update(value: JsValue) -> Result<(), JsValue> {
        let instance: Interface = serde_wasm_bindgen::from_value(value)
    ...
    /// Checks if a Interface exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all Interface instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<Interface> = INTERFACE_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of Interface instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        INTERFACE_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all Interface instances
    #[wasm_bindgen]
    pub fn clear_all() {
        INTERFACE_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds Interface instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: Interface = serde_wasm_bindgen::from_value(instance_js)
    ...
    pub fn add_owned_operation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Ok(target_js) = Operation::get(ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<Operation>(target_js) {
                target.interface = Some(instance_id);
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
    ...
    /// Adds a Interface to redefined_interface
    #[wasm_bindgen]
    pub fn add_redefined_interface(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Interface = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if instance.redefined_interface.contains(&ref_id) {
            return Ok(false);
        }

        instance.redefined_interface.push(ref_id.clone());
    ...
    /// Removes a Interface from redefined_interface
    #[wasm_bindgen]
    pub fn remove_redefined_interface(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Interface = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        if let Some(pos) = instance.redefined_interface.iter().position(|x| x == &ref_id) {
            instance.redefined_interface.remove(pos);
    ...
    /// Clears all Interface from redefined_interface
    #[wasm_bindgen]
    pub fn clear_redefined_interface(instance_id: String) -> Result<usize, JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Interface = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        let count = instance.redefined_interface.len();

        instance.redefined_interface.clear();
...
impl Interface {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate all redefined_interface references exist
        for id in &self.redefined_interface {
            if !Interface::exists(id.clone()) {
                errors.push(format!("Interface {} not found", id));
            }
        }
        ...
        if let Some(id) = &self.protocol {
            if !ProtocolStateMachine::exists(id.clone()) {
                errors.push(format!("ProtocolStateMachine {} not found", id));
            }
        }
