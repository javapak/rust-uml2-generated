...
lazy_static! {
    static ref PROFILE_REGISTRY: Mutex<RefCell<HashMap<String, Profile>>> = 
...
impl Profile {
    ...
    /// Gets all Profile instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<Profile> = PROFILE_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of Profile instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        PROFILE_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Finds Profile instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: Profile = serde_wasm_bindgen::from_value(instance_js)
    ...
    /// Adds a ElementImport to metaclass_reference
    #[wasm_bindgen]
    ...
    /// Removes a ElementImport from metaclass_reference
    #[wasm_bindgen]
    pub fn remove_metaclass_reference(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(pos) = instance.metaclass_reference.iter().position(|x| x == &ref_id) {
            instance.metaclass_reference.remove(pos);
    ...
    /// Clears all ElementImport from metaclass_reference
    #[wasm_bindgen]
    ...
    /// Adds a PackageImport to metamodel_reference
    #[wasm_bindgen]
    ...
    /// Removes a PackageImport from metamodel_reference
    #[wasm_bindgen]
    pub fn remove_metamodel_reference(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        if let Some(pos) = instance.metamodel_reference.iter().position(|x| x == &ref_id) {
            instance.metamodel_reference.remove(pos);
    ...
    /// Clears all PackageImport from metamodel_reference
    #[wasm_bindgen]
...
impl Profile {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // Validate all metaclass_reference references exist
        for id in &self.metaclass_reference {
            if !ElementImport::exists(id.clone()) {
                errors.push(format!("ElementImport {} not found", id));
            }
        }

        // Validate all metamodel_reference references exist
        for id in &self.metamodel_reference {
            if !PackageImport::exists(id.clone()) {
                errors.push(format!("PackageImport {} not found", id));
            }
        }
