...
lazy_static! {
    static ref PSEUDOSTATE_REGISTRY: Mutex<RefCell<HashMap<String, Pseudostate>>> = 
...
#[wasm_bindgen]
impl Pseudostate {
    /// Creates a new Pseudostate and returns its ID
    #[wasm_bindgen]
    pub fn create(kind: PseudostateKind) -> String {
        ...
        PSEUDOSTATE_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Updates a Pseudostate instance
    /// Takes a JavaScript object and updates the registry
    #[wasm_bindgen]
    pub fn update(value: JsValue) -> Result<(), JsValue> {
        let instance: Pseudostate = serde_wasm_bindgen::from_value(value)
    ...
    /// Checks if a Pseudostate exists by ID
    #[wasm_bindgen]
    ...
    /// Gets all Pseudostate instances
    /// Returns an array of JavaScript objects
    #[wasm_bindgen]
    pub fn get_all() -> Result<JsValue, JsValue> {
        let instances: Vec<Pseudostate> = PSEUDOSTATE_REGISTRY.lock().unwrap()
            .borrow()
            .values()
            .cloned()
            .collect();
    ...
    /// Returns the count of Pseudostate instances
    #[wasm_bindgen]
    pub fn count() -> usize {
        PSEUDOSTATE_REGISTRY.lock().unwrap()
            .borrow()
    ...
    /// Removes all Pseudostate instances
    #[wasm_bindgen]
    pub fn clear_all() {
        PSEUDOSTATE_REGISTRY.lock().unwrap()
            .borrow_mut()
    ...
    /// Finds Pseudostate instances by name pattern
    ...
    pub fn add_e_annotation(instance_id: String, ref_id: String) -> Result<bool, JsValue> {
        ...
        let mut instance: Pseudostate = serde_wasm_bindgen::from_value(instance_js)
    ...
    pub fn set_state_machine(instance_id: String, ref_id: Option<String>) -> Result<(), JsValue> {
        ...
        if let Some(old_ref_id) = &instance.state_machine {
            if let Ok(target_js) = StateMachine::get(old_ref_id.clone()) {
                if let Ok(mut target) = serde_wasm_bindgen::from_value::<StateMachine>(target_js) {
                    target.connection_point.retain(|x| x != &instance_id);
                    if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
        ...
        if let Ok(target_js) = StateMachine::get(new_ref_id.clone()) {
            if let Ok(mut target) = serde_wasm_bindgen::from_value::<StateMachine>(target_js) {
                if !target.connection_point.contains(&instance_id) {
                    target.connection_point.push(instance_id.clone());
                }
                if let Ok(target_js) = serde_wasm_bindgen::to_value(&target) {
                    let _ = StateMachine::update(target_js);
                }
    ...
    /// Sets the kind field
    #[wasm_bindgen]
    pub fn set_kind(instance_id: String, value: PseudostateKind) -> Result<(), JsValue> {
        let instance_js = Self::get(instance_id.clone())?;
        let mut instance: Pseudostate = serde_wasm_bindgen::from_value(instance_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
