...
pub struct Behavior {
    ...
    owned_behavior: Vec<Box<Behavior>>,
...
impl Behavior {
    ...
    /// Returns a reference to owned_behavior
    pub fn owned_behavior(&self) -> &Vec<Box<Behavior>> {
        &self.owned_behavior
    }

    /// Returns a mutable reference to owned_behavior
    pub fn owned_behavior_mut(&mut self) -> &mut Vec<Box<Behavior>> {
        &mut self.owned_behavior
    }

    /// Adds an item to owned_behavior
    pub fn add_owned_behavior(&mut self, item: Behavior) {
        self.owned_behavior.push(Box::new(item));
    }
